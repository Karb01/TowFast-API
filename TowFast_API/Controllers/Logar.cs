...
namespace TowFast_API.Controllers
{
    ...
    public class LogarController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("GetpreSolicitacao")]
        public IActionResult GetpreSolicitacao(string id_guincho)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id_guincho))
                {
                    return BadRequest(new { message = "O parâmetro 'id_guincho' é obrigatório." });
                }

                string connectionString = _configuration.GetConnectionString("SqlTowFast");
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("getPreSolicitacao", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;

                        // Adicione o parâmetro à stored procedure
                        command.Parameters.AddWithValue("@id_guincho", id_guincho);
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                result.Add(new
                                {
                                    id = reader["Id_Solicitacao"].ToString(),
                                    Distancia = reader["Distancia"].ToString(),
                                    Preco = reader["Preco"].ToString(),
                                    LatLongCliente = reader["LatLongCliente"].ToString(),
        ...
        [HttpGet]
        [Route("GetPopUpSolicitacao")]
        public IActionResult GetPopUpSolicitacao(string id_guincho)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(id_guincho))
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("PopUpSolicitacao", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("@id_guincho", id_guincho);
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                try
                                {
                                    var item = new
                                    {
                                        Id_Solicitacao = reader["Id_Solicitacao"]?.ToString() ?? "",
                                        Nome_Motorista = reader["Nome_Motorista"]?.ToString() ?? "",
                                        Telefone_Motorista = reader["Telefone_Motorista"]?.ToString() ?? "",
                                        Distancia = reader["Distancia"]?.ToString() ?? "0",
                                        Preco = reader["Preco"]?.ToString() ?? "0",
                                        LatLongCliente = reader["LatLongCliente"]?.ToString() ?? "",
                                        Destino = reader["Destino"]?.ToString() ?? "",
                                ...
                                {
                                    throw new Exception($"Erro ao ler campo da stored procedure: {ex.Message}");
                                }
                            }
                            return Ok(result);
                        }
            ...
            {
                return StatusCode(500, new { message = "Erro ao buscar solicitações do popup.", error = ex.Message });
            }
        }

        [HttpPost("GetSolicitacaoById")]
        public IActionResult GetSolicitacaoById([FromBody] SolicitacaoIdRequest request)
        {
            try
            {
                if (request == null || string.IsNullOrWhiteSpace(request.Id_Solicitacao))
                {
                    return BadRequest(new { message = "O parâmetro 'id_solicitacao' é obrigatório." });
                }

                string connectionString = _configuration.GetConnectionString("SqlTowFast");
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("SELECT * FROM preSolicitacao WHERE Id_Solicitacao = @id_solicitacao", connection))
                    {
                        command.Parameters.AddWithValue("@id_solicitacao", request.Id_Solicitacao);
                        connection.Open();
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                {
                                    Id_Solicitacao = reader["Id_Solicitacao"].ToString(),
                                    Id_Motorista = reader["Id_Motorista"].ToString(),
                                    Id_Guincho = reader["Id_Guincho"].ToString(),
                                    Distancia = reader["Distancia"].ToString(),
                                    Preco = reader["Preco"].ToString(),
                                    LatLongCliente = reader["LatLongCliente"].ToString(),
                                    LatLongGuincho = reader["LatLongGuincho"].ToString(),
                                    Status = reader["Status"].ToString(),
                                    Dta_Solicitacao = reader["Dta_Solicitacao"].ToString(),
                                    Destino = reader["Destino"].ToString()
                                };
            ...
            {
                return StatusCode(500, new { message = "Erro ao buscar a solicitação.", error = ex.Message });
            }
        }

        [HttpPut("AtualizaStatusPreSolicitacao")]
        public IActionResult AtualizaStatusPreSolicitacao([FromBody] AtualizaStatusPreSolicitacaoRequest request)
        {
            try
            {
                string connectionString = _configuration.GetConnectionString("SqlTowFast");
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string updateQuery = "UPDATE preSolicitacao SET Status = @Status WHERE Id_Solicitacao = @Id_Solicitacao";
                    using (SqlCommand command = new SqlCommand(updateQuery, connection))
                    {
                        ...
                        if (rowsAffected == 0)
                        {
                            return NotFound(new { message = "Pré-solicitação não encontrada." });
                        }

                        return Ok(new { message = "Status da pré-solicitação atualizado com sucesso." });
                    }
                }
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { message = "Erro ao atualizar status da pré-solicitação.", error = ex.Message });
            }
        }
    ...
    public class AtualizaStatusPreSolicitacaoRequest
    {
